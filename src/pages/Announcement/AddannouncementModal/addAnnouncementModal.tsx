...
interface AddAnnouncementModalProps {
  onClose: () => void;
  onSuccess: () => void;
  editData?: any; // <--- 1. Added this prop to receive data
}

const API_URL = 'http://localhost:5000/api/announcements';

const AddAnnouncementModal: React.FC<AddAnnouncementModalProps> = ({ onClose, onSuccess, editData }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // --- 3. SWITCH BETWEEN POST (CREATE) AND PUT (UPDATE) ---
      let url = API_URL;
      let method = 'POST';
